...
public class Main {
    public static void main(String[] args){
        ...
        DLL<DLL<Integer>> dll = new DLL<>();

        int n = scanner.nextInt();
        int m = scanner.nextInt();

        for(int i=0; i<n; i++){
            DLL<Integer> dlll = new DLL<>();
            for(int j=0; j<m; j++){
                dlll.insertLast(scanner.nextInt());
            }
            dll.insertLast(dlll);
        }

        DLLNode<DLL<Integer>> curr = dll.getFirst();

        long product = 1;
        while(curr != null){
            int sum = 0;
            DLLNode<Integer> currr = curr.element.getFirst();
            while(currr != null){
                sum+=currr.element;
                currr = currr.succ;
            }
