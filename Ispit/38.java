    ...
    public static void main(String[] args){
        ...
        for(int i=0; i<n+q; i++){
            ...
            if(command.equals("insert")){
                ...
                System.out.println(countDepth(tree.getRoot(), value));
            }
        }
    }

    public static int countDepth(BNode<Integer> node, int value){
        if(node == null){
            return -1;
        }
        if(node.info == value) {
        ...
        int left = countDepth(node.left,value);
        if(left!=-1){
            return 1 + left;
        }

        int right = countDepth(node.right,value);
        if(right!=-1){
            return 1 + right;
        }
