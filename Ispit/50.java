...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            vertices.putIfAbsent(source,0);
            vertices.putIfAbsent(destination,0);
        ...
        int count = DFS(graph,v,v,m);

        System.out.println(count);

    }

    public static int DFS(AdjacencyListGraph<Integer> graph, int startVertex, int sum, int requiredSum) {
        if(sum == requiredSum){
            return 1;
        }
        if(sum > requiredSum){
        ...
        for(int vertex : graph.getNeighbors(startVertex)){
            count += DFS(graph,vertex,sum+vertex,requiredSum);
        }
        return count;
    }
}
...
class AdjacencyListGraph<T> {
    ...
    public void pathsOfLengthNUtil(T vertex, int goalLength, Set<T> visited, List<T> currentPath) {
