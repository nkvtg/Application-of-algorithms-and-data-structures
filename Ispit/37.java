    ...
    public static void main(String[] args){
        ...
        for(int i=0; i<n+q; i++){
            ...
            if(command.equals("root")){
                ...
                System.out.println(countSumOfDegrees(node));
            }
        }
    }

    public static int countSumOfDegrees(BNode<String> node){
        if(node == null || (node.left == null && node.right == null)){
            return 0;
        }
        
        if(node.left!=null && node.right!=null){
            return 2 + countSumOfDegrees(node.left) + countSumOfDegrees(node.right);
        } else {
            if(node.left!=null){
                return 1 + countSumOfDegrees(node.left);
            }
            if(node.right!=null){
                return 1 + countSumOfDegrees(node.right);
            }
        }
