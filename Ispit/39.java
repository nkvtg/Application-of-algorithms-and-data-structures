    ...
    public static void main(String[] args){
        ...
        for(int i=0; i<n+q; i++){
            ...
            if(command.equals("insert")){
                ...
                System.out.println(countSmaller(tree.getRoot(), value));
            }
        }
    }

    public static int countSmaller(BNode<Integer> node, int value){
        if(node == null){
            return 0;
        }
        if(node.info < value){
            return 1 + countSmaller(node.left,value) + countSmaller(node.right,value);
        } else {
