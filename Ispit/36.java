    ...
    public static void main(String[] args){
        ...
        for(int i=0; i<n+q; i++){
            ...
            if(command.equals("root")){
                ...
                System.out.println(countMaxDepth(node));
            }
        }
    }

    public static int countMaxDepth(BNode<String> node){
        if(node == null){
            return 0;
        }

        int max = 0;
        max = Math.max(max, countMaxDepth(node.left));
        max = Math.max(max, countMaxDepth(node.right));
