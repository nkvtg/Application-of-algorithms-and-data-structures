    ...
    public static void main(String[] args){
        ...
        for(int i=0; i<n+q; i++){
            ...
            if(command.equals("insert")){
                ...
                System.out.println(countBigger(tree.getRoot(), value));
            }
        }
    }

    public static int countBigger(BNode<Integer> node, int value){
        if(node == null){
            return 0;
        }

        if(node.info > value){
            return 1 + countBigger(node.left,value) + countBigger(node.right,value);
        } else {
            return countBigger(node.right,value);
        }
    }
}
